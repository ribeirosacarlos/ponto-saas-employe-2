// src/i18n/locales/en.js
const en = {
  translation: {
    common: {
      brand: 'Jornafy',
      brandSubtitle: 'Secure journeys aligned with labor law',
      connectedBadge: 'Connected to backend',
      realtime: 'Live',
      actions: {
        cancel: 'Cancel',
        submit: 'Submit',
        close: 'Close',
        logout: 'Sign out',
      },
      language: {
        label: 'Language',
        pt: 'PT',
        en: 'EN',
        es: 'ES',
      },
      loading: 'Loading...',
      sourceFallback: 'web',
      activeTimezone: 'Timezone: {{tz}}',
    },
    branding: {
      name: 'Jornafy',
      subtitle: 'Time & attendance platform',
      logoAlt: 'Jornafy logo',
    },
    supportContact: {
      title: 'Contact support',
      description: 'Use one of the contacts below to get help.',
      emailLabel: 'Email',
      phoneLabel: 'WhatsApp',
    },
    themeToggle: {
      ariaLabel: 'Toggle theme',
      lightLabel: 'Light theme',
      darkLabel: 'Dark theme',
    },
    languageSwitcher: {
      ariaLabel: 'Change language',
    },
    errors: {
      forbidden: {
        title: 'Access denied',
        description: 'You do not have permission to perform this action.',
      },
    },

    settingsPage: {
      header: {
        title: 'Settings',
        subtitle: 'Plan, company and preferences',
        actions: {
          reload: 'Reload',
          manageSubscription: 'Manage subscription',
          updatePlan: 'Update plan',
          portalUnavailable: 'Portal unavailable',
        },
      },
      states: {
        empty: 'No settings information returned.',
      },
      errors: {
        load: 'Unable to load settings.',
        forbidden: 'You do not have permission to view settings.',
        noCompany: 'No company associated to the user.',
      },
      plan: {
        title: 'Plan summary',
        subtitle: 'Subscription status and company usage.',
        unknown: 'Unknown plan',
        empty: 'No plan information available.',
        statusLabel: 'Status: {{status}}',
        trialEnds: 'Trial until {{date}}',
        renews: 'Renews on {{date}}',
        labels: {
          price: 'Price',
          interval: 'Cycle',
          periodFallback: 'Next cycle',
          trialEnds: 'Trial ends',
          nextCharge: 'Next charge',
          periodEnds: 'Period ends',
          nextAction: 'Next action',
          trialDays: 'Trial days',
          billingDays: 'Billing days',
          cancelAtPeriodEnd: 'Cancel at period end',
          endsAt: 'Ends at',
          yes: 'Yes',
          no: 'No',
          unknownInterval: 'Unknown interval',
          month: 'Month',
          year: 'Year',
          oneTime: 'One-time',
          noPrice: 'No price',
          limits: 'Limits',
        },
        usage: {
          withLimit: '{{current}} of {{limit}} users counted in the plan',
          withoutLimit: '{{current}} users counted in the plan',
          overLimit: 'Over the limit',
          percent: '{{percent}}% of capacity',
        },
        actions: {
          portal: 'Customer portal',
          portalDisabled: 'Portal unavailable',
          viewDetails: 'View details',
          hideDetails: 'Hide details',
        },
        status: {
          unknown: 'Unknown status',
        },
        extraEmployees: {
          pendingTitle: 'Pending payment for extra counted users',
          overdueTitle: 'Overdue payment for extra counted users',
          pendingDescription: 'There are {{count}} extra counted user(s) waiting for payment before becoming purchased capacity.',
          badges: {
            pending: 'Pending',
            overdue: 'Overdue',
          },
          fields: {
            pendingQuantity: 'Pending quantity',
            dueAt: 'Payment due date',
            paidAllowance: 'Paid extra capacity',
          },
          notes: {
            pendingKeepsQuantity: 'Removing users before payment does not reduce the open pending quantity.',
            pendingAllowsGrace: 'While still within the deadline, the company can keep up to 3 pending extra counted users.',
            overdueBlocksNewEmployees: 'Once overdue, new extra plan usage may be blocked until payment is completed.',
          },
          actions: {
            payNow: 'Pay extras in Stripe',
            redirecting: 'Redirecting...',
          },
          toast: {
            checkoutErrorTitle: 'Could not open checkout',
          },
          errors: {
            checkout: 'An error occurred while starting payment for extra counted users.',
            missingUrl: 'Checkout response did not include a valid URL.',
          },
        },
      },
      security: {
        title: 'Security',
        heading: 'Accounts and access',
        mfa: 'Two-factor authentication',
        mfaHint: 'Require MFA for admins and managers.',
        sessions: 'Active sessions',
        sessionsHint: 'Revoke suspicious or expired sessions.',
      },
      subscription: {
        title: 'Manage subscription',
        heading: 'Manage subscription',
        subtitle: 'Track the current status and stop renewal when needed.',
        loadingTitle: 'Loading subscription',
        errorTitle: 'Unable to load subscription',
        notAvailable: '--',
        yes: 'Yes',
        no: 'No',
        status: {
          active: 'Active',
          canceling: 'Canceling',
          canceled: 'Canceled',
        },
        fields: {
          renewal: 'Renewal',
          expiration: 'Expiration',
          scheduledCancellation: 'Scheduled cancellation',
        },
        accessUntil: 'Your access remains available until {{date}}',
        accessUntilFallback: 'Your access remains available until the current period ends.',
        scheduledTitle: 'Subscription scheduled for cancellation',
        scheduledAccessUntil: 'Access until {{date}}',
        actions: {
          cancel: 'Cancel subscription',
          cancelling: 'Canceling...',
        },
        dialog: {
          title: 'Cancel subscription',
          description: 'Confirm that you want to stop automatic renewal for this subscription.',
          keepAccess: 'When you cancel, your subscription will remain active until the end of the paid period.',
          blockAfter: 'After that date, access will be blocked.',
          confirm: 'Confirm cancellation',
        },
        toast: {
          successTitle: 'Subscription scheduled for cancellation',
          successDescription: 'Access until {{date}}',
          successDescriptionFallback: 'The subscription was scheduled for cancellation.',
          errorTitle: 'Unable to cancel subscription',
          errorDescription: 'An error occurred while requesting subscription cancellation.',
        },
      },
      preferences: {
        title: 'Preferences',
        subtitle: 'Company timezone, punch access and geolocation.',
        helper: 'Applied to filters, times and exports.',
        fields: {
          timezone: 'Company timezone',
          locale: 'Language',
          timezonePlaceholder: 'Type to search and select',
        },
        clockDevice: {
          title: 'Allowed device for punch registration',
          description:
            'Choose where employees can register punches. At least one option must stay enabled.',
          helper: 'At least one registration method must remain enabled.',
          options: {
            mobile: 'Mobile app',
            desktop: 'Desktop browser',
          },
          states: {
            loading: 'Loading device settings...',
          },
          validation: {
            atLeastOne: 'At least one device type must be allowed for punch registration.',
          },
          toast: {
            errorDescription: 'Could not save the allowed device settings for punch registration.',
          },
        },
        notificationsTitle: 'Notifications and alerts',
        actions: {
          edit: 'Edit',
          save: 'Save',
          reload: 'Reload',
        },
        states: {
          errorLoading: 'Could not load timezone.',
          noTimezone: 'No timezone configured.',
        },
        toast: {
          successTitle: 'Timezone updated',
          successDescription: 'The company now uses the new timezone.',
          errorTitle: 'Save failed',
          errorDescription: 'Could not save timezone.',
        },
      },
      locationValidation: {
        eyebrow: 'Geolocation',
        title: 'Location validation',
        description:
          'Define the company location and allowed radius to identify records created outside the expected area.',
        helper:
          'This validation does not block punch registration. It only serves future alerts in record views.',
        fields: {
          locationValidationEnabled: 'Enable location validation',
          locationValidationEnabledHint:
            'When enabled, records can be compared against the company base location to generate future alerts.',
          companyLatitude: 'Company latitude',
          companyLongitude: 'Company longitude',
          allowedRadiusMeters: 'Allowed radius in meters',
        },
        placeholders: {
          companyLatitude: 'Example: -23.5505200',
          companyLongitude: 'Example: -46.6333080',
          allowedRadiusMeters: 'Example: 100',
        },
        alerts: {
          incomplete: 'The company location has not been fully configured yet.',
          enabledWithoutCoordinates:
            'Location validation is enabled, but the company latitude and longitude have not been configured yet.',
        },
        actions: {
          save: 'Save',
          saving: 'Saving...',
          reload: 'Reload',
        },
        states: {
          loading: 'Loading location settings...',
          saved: 'Location settings saved successfully.',
          errorTitle: 'Error loading data',
        },
        errors: {
          load: 'Unable to load location settings.',
        },
        validation: {
          latitudeRequired: 'Enter the company latitude.',
          latitudeNumber: 'Latitude must be a valid number.',
          latitudeRange: 'Latitude must be between -90 and 90.',
          longitudeRequired: 'Enter the company longitude.',
          longitudeNumber: 'Longitude must be a valid number.',
          longitudeRange: 'Longitude must be between -180 and 180.',
          radiusRequired: 'Enter the allowed radius.',
          radiusNumber: 'Radius must be a valid number.',
          radiusInteger: 'Radius must be an integer.',
          radiusRange: 'Radius must be between 10 and 5000 meters.',
        },
        toast: {
          successTitle: 'Settings saved',
          successDescription: 'The company location validation settings were updated.',
          errorTitle: 'Error saving',
          errorDescription: 'Unable to save location settings.',
        },
      },
      security: {
        title: 'Security / Compliance',
        subtitle: 'Essential account controls.',
        helper: 'Values returned by the API.',
        fields: {
          twoFactor: '2FA enabled',
          lastLogin: 'Last login',
          logRetention: 'Log retention (days)',
          exportEnabled: 'Export enabled',
        },
        labels: {
          enabled: 'Enabled',
          disabled: 'Disabled',
        },
      },
      advanced: {
        title: 'Advanced details',
        subtitle: 'Technical IDs and URLs',
        helper: 'Use only when you need to copy billing info.',
        planSlug: 'Plan slug',
        stripeCustomerId: 'Stripe Customer ID',
        stripeSubscriptionId: 'Stripe Subscription ID',
        customerPortal: 'Customer portal',
        checkoutUrl: 'Checkout URL',
        subscriptionStatus: 'Subscription status',
        nextAction: 'Next action',
      },
    },
    access: {
      shared: {
        retry: 'Try again',
        details: 'Restriction details',
      },
      companyMissing: {
        badge: 'No company',
        title: 'A company link is required',
        description:
          'We could not find a company associated to your user. Ask an admin to link it or create a company to continue.',
      },
      subscription: {
        badge: 'Subscription required',
        title: 'Activate your plan to continue',
        description: 'Your workspace needs an active subscription or trial to use the app.',
        startCheckout: 'Start checkout',
        genericCheckout: 'Go to checkout',
        monthly: 'Monthly',
        yearly: 'Yearly',
        intervalLabel: 'Interval',
        checkoutErrorTitle: 'Could not start checkout',
        blockedTitle: 'Access blocked',
        blockedDescription: 'Access denied: an active subscription is required to continue.',
        errorFallback: 'Failed to load plans. Please try again shortly.',
        plansTitle: 'Available plans',
        plansSubtitle: 'Pick a plan to activate or renew your subscription.',
        choosePlan: 'Choose this plan',
        currentPlan: 'Current plan',
        recommended: 'Recommended',
        freePlan: 'Free',
        perInterval: {
          month: '/month',
          year: '/year',
          oneTime: 'one-time payment',
        },
        trialLabel: '{{count}}-day trial',
        badgeYearly: 'Yearly plan',
        badgeMonthly: 'Monthly plan',
        perk1: 'Workspace unlocked',
        perk1Desc: 'Clock-ins, adjustments and history are enabled as soon as the plan is active.',
        perk2: 'Secure billing',
        perk2Desc: 'Guided checkout to activate or renew your plan in seconds.',
        errorFallback: 'Failed to load plans. Please try again shortly.',
      },
      forbidden: {
        badge: 'Restricted area',
        title: 'Access forbidden',
        description: 'You do not have permission to view this content.',
      },
    },
    types: {
      in: 'Clock-in',
      out: 'Clock-out',
    },
    dashboard: {
      badge: 'Time clock',
      greeting: 'Hello, {{name}}',
      fallbackName: 'teammate',
      description: 'Punch in/out, request adjustments, and track your history in real time.',
      sectionLabel: 'My time',
      summary: 'Punch summary',
      myPoint: 'My punches',
      details: 'Details',
      menu: {
        history: 'Punch history',
        signOut: 'Sign out',
        theme: 'Toggle theme',
        comingSoon: 'Page not available yet.',
      },
      tokenActive: 'Active token',
      lastPunch: 'Last punch',
      registeredAt: 'Registered at: {{time}}',
      firstPunch: 'First punch',
      interval: 'Break',
      lastPunchRow: 'Last punch',
      openStatus: 'Open',
      todayHistory: 'Today history',
      noEntriesToday: 'No punches recorded today',
      lastPunchFallback: 'No punches recorded today',
      lastPunchType: '{{time}} - {{type}}',
      nowLabel: 'now',
      requestAdjustment: 'Request adjustment',
      hoursToday: 'Hours worked today',
      hoursTodayHelper: "Server-calculated hours from today's punches.",
      hourBank: 'Time bank',
      hourBankHelper: 'Static value until the endpoint is connected.',
      backendNote:
        'The backend expects alternating {{inLabel}} and {{outLabel}}. The next send will be {{nextType}}.',
      registerButton: 'Register punch',
      registering: 'Registering...',
      nextLabel: {
        in: 'Register clock-in',
        out: 'Register clock-out',
      },
      employeeCardTitle: 'Team member data',
      name: 'Name',
      email: 'Email',
      role: 'Role',
      roleValue: 'Employee',
    },
    sidebar: {
      sections: {
        workspace: 'Workspace',
        admin: 'Administration',
        commercial: 'Commercial',
        affiliate: 'Affiliate Panel',
        superAdmin: 'Super Admin',
      },
      groups: {
        pointManagement: 'Time management',
        team: 'Team',
        content: 'Content',
        myRecords: 'My records',
      },
      helpCenter: 'Help center',
      actions: {
        expand: 'Expand sidebar',
        collapse: 'Collapse sidebar',
        language: 'Language',
        theme: 'Theme',
      },
      items: {
        dashboard: 'Dashboard',
        history: 'Punch history',
        documents: 'Documents',
        announcements: 'Announcements',
        clock: 'Register point',
        projects: 'Projects',
        team: 'Team',
        platformCompanies: 'Companies',
        platformBillingPlans: 'Billing plans',
        auditLogs: 'Audit logs',
        superAdminDashboard: 'Dashboard',
        superAdminCompanies: 'Companies',
        superAdminBlog: 'Blog',
        settings: 'Settings',
        profile: 'My Profile',
        calendar: 'Calendar',
        areas: 'Areas',
        shifts: 'Shifts',
        adjustments: 'Adjustments',
        closeTimesheet: 'Close timesheet',
        monthlyClosures: 'Monthly closure',
        companyTimezone: 'Company timezone',
        holidays: 'Holidays',
        commercialDashboard: 'Commercial Dashboard',
        commercialLeads: 'Leads',
        commercialSteps: 'Pipeline Stages',
        commercialAffiliates: 'Affiliates',
        commercialCommissions: 'Commissions & Bonuses',
        affiliateOverview: 'Overview',
        affiliateLeads: 'My Leads',
        affiliateCommissions: 'My Commissions',
        affiliateBonuses: 'My Bonuses',
      },
    },
    adminOnboarding: {
      actions: {
        next: 'Next',
        previous: 'Previous',
        done: 'Finish',
      },
      panel: {
        eyebrow: 'First steps',
        title: 'Shortcuts to set up the admin workspace',
        subtitle: 'Centralize the operational setup here and replay the tutorial whenever you need.',
        restart: 'Replay tutorial',
        employees: {
          title: 'Employees',
          description: 'Open the full team management area and keep employee records organized.',
        },
        addEmployee: {
          title: 'Add employee',
          description: 'Start the operational setup by creating the company’s first employee.',
        },
        shifts: {
          title: 'Shifts',
          description: 'Configure schedules, default hours and rules used by time tracking.',
        },
        reports: {
          title: 'Reports',
          description: 'Monitor adjustments and quickly review what needs attention.',
        },
      },
      steps: {
        dashboardHeader: {
          title: 'Welcome to the admin workspace',
          description: 'This is where you follow the company operation, open key shortcuts and monitor the most important daily signals.',
        },
        menuEmployees: {
          title: 'Employee management',
          description: 'Use this shortcut to access the employee area, review the team and keep records organized.',
        },
        addEmployee: {
          title: 'Create the first employee',
          description: 'This step takes you straight to the onboarding flow to start building the company base in the system.',
        },
        workSchedule: {
          title: 'Set up work schedules',
          description: 'Define shifts, hours and rules that will be used in employee time tracking.',
        },
        reports: {
          title: 'Reports and follow-up',
          description: 'Track adjustments, movements and indicators so you can react quickly when something needs review.',
        },
        final: {
          title: 'All set',
          description: 'Your workspace is ready. Whenever you want to review this tour, use the "Replay tutorial" button on the dashboard.',
        },
      },
    },
    employeeOnboarding: {
      actions: {
        next: 'Next',
        previous: 'Previous',
        done: 'Finish',
      },
      panel: {
        eyebrow: 'First steps',
        title: 'Your daily workspace',
        subtitle: 'Use this area to clock in, check your records and review adjustments.',
        restart: 'Replay tutorial',
        clock: {
          title: 'Clock in',
          description: 'Open the clock screen and register your punch.',
        },
        history: {
          title: 'View history',
          description: 'Check your records whenever you need.',
        },
        adjustments: {
          title: 'Request adjustment',
          description: 'Use this shortcut to review or correct a record.',
        },
      },
      steps: {
        home: {
          title: 'Welcome',
          description: 'This is your space to follow your time routine.',
        },
        clock: {
          title: 'Clock in',
          description: 'Tap here to open the screen and register clock-in or clock-out.',
        },
        history: {
          title: 'Check records',
          description: 'This is where you review your times and daily history.',
        },
        adjustments: {
          title: 'Request adjustment',
          description: 'If something needs correction, start from this shortcut.',
        },
        final: {
          title: 'Done',
          description: 'You are set. To review the tour later, use the dashboard button.',
        },
      },
    },
    superAdmin: {
      nav: {
        section: 'Super Admin',
      },
      blog: {
        title: 'Blog',
      },
      shared: {
        values: {
          yes: 'Yes',
          no: 'No',
        },
        actions: {
          refresh: 'Refresh',
          retry: 'Try again',
        },
        states: {
          errorTitle: 'Something went wrong',
        },
        errors: {
          forbidden: 'You do not have permission to access this area.',
          unauthorized: 'Your session expired. Sign in again.',
          generic: 'We could not complete the request.',
        },
        health: {
          healthy: 'Healthy',
          warning: 'Warning',
          critical: 'Critical',
          unknown: 'Unknown',
        },
        subscription: {
          active: 'Active',
          blocked: 'Blocked',
          trialing: 'Trial',
          past_due: 'Past due',
          canceled: 'Canceled',
          unknown: 'Unknown',
        },
      },
      dashboard: {
        title: 'Platform dashboard',
        subtitle: 'Consolidated view of company operations, subscriptions, and activity.',
        generatedAt: 'Generated at {{value}}',
        metrics: {
          companiesTotal: 'Total companies',
          companiesActive30d: 'Active companies 30d',
          companiesTrial: 'Trial companies',
          companiesPaying: 'Paying companies',
          companiesPastDue: 'Past due companies',
          companiesAtRisk: 'At-risk companies',
          employeesTotal: 'Total employees',
          employeesActive30d: 'Active employees 30d',
          timeEntriesToday: 'Time entries today',
          timeEntries30d: 'Time entries 30d',
          mrr: 'Estimated MRR',
          averageTicket: 'Average ticket',
        },
        hero: {
          eyebrow: 'Platform view',
          title: 'Super admin executive summary',
          description:
            'Track base expansion, billing health, recent activity, and risk signals using only the data already delivered by the platform.',
          badge: 'Consolidated snapshot',
          highlights: {
            newCompanies: 'New companies 30d',
            newCompaniesHint: 'Recent base growth',
            active7d: 'Active companies 7d',
            active7dHint: 'Usage observed during the last week',
            expiring: 'Expiring in 7 days',
            expiringHint: 'Renewals that need follow-up',
          },
        },
        blocks: {
          subscriptionsTag: 'Subscriptions',
          subscriptionsTitle: 'Billing health',
          subscriptionsDescription: 'Current monetization status, billing risk, and near-term renewals.',
          trialing: 'Companies in trial',
          paying: 'Paying companies',
          pastDue: 'Past due companies',
          expiring: 'Expiring in 7 days',
          averageTicket: 'Average ticket',
          averageTicketHint: 'Estimated average value based on current subscriptions.',
          operationsTag: 'Operations',
          operationsTitle: 'Platform usage',
          operationsDescription: 'Aggregated activity readings already observable from the current endpoint.',
          activeCompanies7d: 'Active companies 7d',
          activeCompanies30d: 'Active companies 30d',
          timeEntries7d: 'Time entries 7d',
          newCompanies30d: 'New companies 30d',
          riskTag: 'Risk',
          riskTitle: 'Sensitive indicators',
          riskDescription: 'Items that require closer manual follow-up by the super admin.',
          blockedCompanies: 'Blocked companies',
          atRiskCompanies: 'At-risk companies',
          totalEmployees: 'Total employees',
          activeEmployees30d: 'Active employees 30d',
        },
        revenue: {
          eyebrow: 'Revenue',
          title: 'Estimated revenue',
          description: 'Consolidated view of the monetary indicators returned by the API.',
          mrr: 'Estimated MRR',
          averageTicket: 'Average ticket',
          currency: 'Currency',
        },
        notes: {
          eyebrow: 'Readings and limits',
          title: 'Operational notes',
          description: 'Important context to correctly interpret the figures shown on this page.',
          valueEstimated: 'Estimated',
          valueBilling: 'Billing',
          mrrTitle: 'MRR and average ticket',
          mrrDescription: 'The values shown are revenue estimates, not settled billing.',
          employeeTitle: 'Employee model',
          employeeDescription:
            'The count should reflect billable users and users with time entries, not only profiles with the employee role.',
        },
        snapshot: {
          eyebrow: 'Snapshot',
          title: 'Quick read',
          description: 'Short summary of the platform’s main volume indicators in this view.',
          companies: 'Companies',
          employees: 'Employees',
          timeEntries: 'Time entries 30d',
          revenue: 'Current MRR',
        },
        backendCoverage: {
          eyebrow: 'Backend coverage',
          title: 'What the current API already provides',
          description:
            'This dashboard uses only the current /v1/platform/super-admin/dashboard payload and makes clear what is still missing for richer blocks.',
          sections: {
            available: 'Available now',
            missing: 'Not covered yet',
            nextPackage: 'Next minimum package',
          },
          available: {
            generatedAt: 'Snapshot generation timestamp',
            companies: 'Aggregated company counters',
            employees: 'Billable/active employees',
            timeEntries: 'Time entries for today, 7 days, and 30 days',
            revenue: 'Estimated MRR, average ticket, and currency',
          },
          missing: {
            series: 'Time series for heatmaps and trend charts',
            operationalLists: 'Operational lists with top companies and recent changes',
            breakdowns: 'Breakdowns by plan, status, and risk buckets',
          },
          nextPackage: {
            timeSeries: '30/60/90 day series for time entries and active companies',
            rankings: 'Rankings for highest risk, highest activity, and fastest growth companies',
            statusDistribution: 'Distribution by subscription status and by plan',
            recentEvents: 'Recent events such as creation, blocking, past due, and plan change',
          },
        },
        states: {
          emptyTitle: 'No data available',
          emptyDescription: 'The dashboard has not returned any indicators to display yet.',
          errorDescription: 'Could not load the dashboard.',
        },
      },
      companyDetails: {
        administrativeSettings: {
          tag: 'Settings',
          title: 'Administrative Settings',
          description:
            'Control the company timezone and audit visibility for company administrators.',
          helper:
            'Changes apply to the selected company and follow the platform backend validations.',
          summary: {
            timezone: 'Current timezone',
            audit: 'Audit for admins',
          },
          fields: {
            timezoneLabel: 'Company timezone',
            timezoneHint:
              'Use the API-provided list to define how the company dates and times should be interpreted.',
            auditLabel: 'Allow audit log access for company admins',
            auditHint:
              'When disabled, only the super admin will be able to consult audit logs.',
          },
          values: {
            enabled: 'Allowed',
            disabled: 'Blocked',
          },
          actions: {
            save: 'Save',
            saving: 'Saving...',
            reload: 'Reload',
          },
          states: {
            errorTitle: 'Error loading settings',
            saved: 'Administrative settings saved successfully.',
          },
          toast: {
            successTitle: 'Settings saved',
            successDescription: 'The company administrative settings were updated.',
            errorTitle: 'Error saving',
            errorDescription: 'Unable to save the administrative settings.',
          },
          validation: {
            atLeastOne: 'Change at least one setting before saving.',
          },
        },
      },
    },
    adminShiftsPage: {
      title: 'Work shifts',
      subtitle: 'Configure work windows, daily workload, and breaks for each shift.',
      actions: {
        refresh: 'Refresh',
        create: 'New shift',
      },
      states: {
        noPermission: 'You do not have permission to manage shifts.',
        emptyTitle: 'No shifts created yet',
        emptyDescription: 'Create standard or flexible shifts to organize your team schedule.',
      },
      dialog: {
        createTitle: 'New work shift',
        editTitle: 'Edit work shift',
        description: 'Set the work window, daily workload per day, and breaks.',
        deleteDescription:
          'This action will remove the shift "{{name}}". Employees linked to it will need reassignment.',
        deleteConfirm: 'Delete shift',
        selectedFallback: 'Shift',
      },
      form: {
        name: 'Shift name',
        namePlaceholder: 'e.g. Standard shift (Mon-Fri)',
        default: 'Default shift',
        flexible: 'Flexible',
        daysHelper: 'Adjust all 7 days of the week',
        dayWorking: 'Set schedule and break',
        dayOff: 'Day off',
        dayActive: 'Active day',
        startLabel: 'Start',
        endLabel: 'End',
        scheduledMinutesLabel: 'Daily workload',
        breakStartLabel: 'Break start',
        breakEndLabel: 'Break end',
        breakMinutesLabel: 'Break duration',
        infoTitle: 'Work window and daily workload are different things now',
        infoDescription:
          'The window defines when the employee may work. The daily workload defines how much they need to complete that day. Example: shift 11:00-19:00 with daily workload 06:00.',
        templateTitle: 'Use the same base setup during the week',
        templateDescription:
          'Fill the window, daily workload, and break once. Then customize only the days that differ.',
        sameAsWeek: 'Same weekly setup',
        customizeDay: 'Edit this day only',
        customDay: 'Custom day',
        windowSummaryLabel: 'Window',
        workloadSummaryLabel: 'Daily workload',
        breakSummaryLabel: 'Break',
        validation: {
          daysCount: 'Send all 7 days of the week.',
          invalidWeekday: 'The day identifier must be between 1 and 7.',
          duplicateWeekday: 'Each weekday must appear only once.',
          startEndRequired: 'Provide start and end times for active days.',
          invalidWindow: 'The start time must be earlier than the end time.',
          breakPair: 'Fill break start and end together.',
          breakOrder: 'The break must start before it ends.',
          breakInsideWindow: 'The break must stay inside the shift window.',
          invalidWorkload: 'Daily workload must be a valid HH:MM value.',
          workloadExceedsWindow: 'Daily workload cannot be greater than the shift window.',
        },
      },
      toasts: {
        nameRequired: {
          title: 'Enter a shift name',
          description: 'Provide a name to identify this shift.',
        },
        created: {
          title: 'Shift created',
          description: 'New shift available for assignments.',
        },
        updated: {
          title: 'Shift updated',
          description: 'Changes have been saved.',
        },
        saveError: {
          title: 'Save error',
          description: 'We could not save this shift. Try again.',
        },
        validation: {
          title: 'Review the shift data',
          description: 'We found invalid fields before saving.',
        },
        deleted: {
          title: 'Shift removed',
          description: 'The shift "{{name}}" has been deleted.',
        },
        deleteError: {
          title: 'Delete error',
          description: 'We could not remove this shift now.',
        },
      },
    },
    adminAreasPage: {
      title: 'Areas',
      subtitle: 'Create and manage company areas.',
      actions: {
        refresh: 'Refresh',
        create: 'New area',
        edit: 'Edit',
        delete: 'Delete',
      },
      card: {
        identifier: 'ID {{id}}',
        employeeCount: '{{count}} employee(s)',
        membersLabel: 'Employees',
        emptyBadge: 'Empty',
        memberCountBadge: '{{count}}',
        moreMembers: '+{{count}} selected',
        emptyMembers: 'No employees linked to this area.',
      },
      metrics: {
        totalAreas: 'Total areas',
        totalEmployees: 'Total employees',
        emptyAreas: 'Areas without employees',
        busiestArea: 'Area with most employees',
        noEmployees: 'None',
      },
      states: {
        noPermissionTitle: 'Restricted access',
        noPermissionDescription: 'Only admins can manage areas.',
        errorTitle: 'Failed to load areas',
        emptyTitle: 'No areas created',
        emptyDescription: 'Create the first area to organize the team.',
        cardFallback: 'Area',
      },
      dialog: {
        createTitle: 'New area',
        editTitle: 'Edit area',
        description: 'Set the area name and manage linked employees in the same flow.',
        deleteTitle: 'Delete area',
        deleteDescription: 'Do you want to delete the area {{name}}?',
      },
      form: {
        nameLabel: 'Area name',
        namePlaceholder: 'e.g. Operations',
        employeesLabel: 'Area employees',
        employeesHint:
          'Use Ctrl or Cmd to select multiple employees. On save, the frontend updates each employee area_id through the existing employee flow.',
        employeesSearchPlaceholder: 'Search employee by name',
        emptySearch: 'No employees found for this search.',
        selectedCount: '{{count}} selected',
        linkedPreviewLabel: 'Selected links',
        employeeFallback: 'Unnamed employee',
      },
      toasts: {
        loadError: {
          title: 'Failed to load areas',
          description: 'We could not load the areas.',
        },
        employeesLoadError: {
          title: 'Failed to load employees',
          description: 'We could not load employees for area linking.',
        },
        nameRequired: {
          title: 'Enter an area name',
          description: 'Provide a name to continue.',
        },
        created: {
          title: 'Area created',
          description: 'The area has been created.',
        },
        updated: {
          title: 'Area updated',
          description: 'Changes saved successfully.',
        },
        saveError: {
          title: 'Failed to save area',
          description: 'We could not save the area.',
        },
        deleted: {
          title: 'Area deleted',
          description: 'The area {{name}} has been removed.',
        },
        deleteError: {
          title: 'Failed to delete area',
          description: 'We could not delete the area.',
        },
      },
    },
    adminTimezonePage: {
      title: 'Company timezone',
      subtitle:
        'Only administrators can change the timezone reference used across lists, filters and exports.',
      actions: {
        reload: 'Reload',
        save: 'Save',
        saving: 'Saving...',
        reset: 'Discard changes',
      },
      form: {
        title: 'Configure timezone',
        fieldLabel: 'Company timezone (for display and filters)',
        fieldHint: 'The backend applies this timezone to lists, reports and exports.',
        placeholder: 'Type or select a timezone',
        tooltip:
          'This value defines how date/time series appear and are interpreted in ASP.NET (Europe/Madrid for Spanish companies).',
        tooltipLabel: 'Help about company timezone',
        autoUtc: 'The API converts everything to UTC internally.',
      },
      filters: {
        title: 'How this impacts filters and exports',
        description:
          'Whenever a date filter is opened (reports, exports), ranges are interpreted using this timezone. The API already converts everything to UTC, so the front just needs to inform users.',
      },
      toast: {
        successTitle: 'Timezone updated',
        successDescription: 'The company now uses the new timezone.',
        errorTitle: 'Save error',
        errorDescription: 'Unable to save the timezone.',
      },
      states: {
        errorLoading: 'Unable to load timezone.',
        noPermissionTitle: 'Admins only',
        noPermissionDescription: 'This panel is restricted to administrator users.',
        noOptions: 'No timezones available',
      },
    },
    adminAdjustmentsPage: {
      title: 'Time adjustments',
      subtitle: 'Review and act on employee adjustment requests.',
      filters: {
        title: 'Filters',
        description: 'Filter by status, employee or search by name/reason.',
        status: 'Status',
        all: 'All',
        employee: 'Employee',
        allEmployees: 'All employees',
        selectedCount: '{{count}} employee(s) selected',
        search: 'Search',
        searchPlaceholder: 'Name, email or reason',
      },
      list: {
        title: 'Adjustment requests',
        description: 'Approve or reject time adjustments sent by the team.',
      },
      table: {
        createdAt: 'Requested at',
        original: 'Original time',
        corrected: 'Corrected time',
        reason: 'Reason',
        unset: 'Not provided',
        id: 'ID',
        userFallback: 'Employee',
        userEmailFallback: 'Email not provided',
      },
      status: {
        pending: 'Pending',
        approved: 'Approved',
        rejected: 'Rejected',
        unknown: 'Unknown',
      },
      actions: {
        refresh: 'Refresh',
        approveSuccess: 'Adjustment approved',
        approveDescription: 'Status updated to approved.',
        approveError: 'Unable to approve now',
        rejectSuccess: 'Adjustment rejected',
        rejectDescription: 'Status updated to rejected.',
        rejectError: 'Unable to reject now',
        rejecting: 'Rejecting...',
        reject: 'Reject',
        approving: 'Approving...',
        approve: 'Approve',
      },
      states: {
        errorTitle: 'Something went wrong',
        errorDescription: 'Unable to load adjustments. Try again shortly.',
        emptyTitle: 'No requests',
        emptyDescription: 'No adjustments found for the selected filters.',
      },
      teamEntries: {
        title: 'Team punches',
        description: 'Latest punches from users in your team.',
        error: 'Unable to load team punches.',
        empty: 'No punches found yet.',
      },
      types: {
        breakStart: 'Break start',
        breakEnd: 'Break end',
      },
      pagination: {
        page: 'Page',
        of: 'of',
        previous: 'Previous',
        next: 'Next',
      },
    },
      toast: {
        sessionExpired: {
          title: 'Session expired',
          description: 'Log in again or use a user with time clock access.',
        },
      fetchEntriesError: {
        title: 'Error fetching punches',
        description: 'Could not load your punches. Try again shortly.',
      },
      clockSuccess: {
        title: 'Punch registered',
        description: 'Saved at {{time}}.',
      },
      clockRateLimit: {
        title: 'Please wait a moment',
      },
      clockValidation: {
        title: 'Unable to register',
      },
      clockError: {
        title: 'Error registering',
        description: 'Could not register the punch. Check permissions or try again.',
      },
      adjustmentSuccess: {
        title: 'Request sent',
        description: 'Your adjustment was recorded as pending for approval.',
      },
      adjustmentError: {
        title: 'Send error',
        description: 'Could not send the adjustment. Check the fields and try again.',
      },
      logout: {
        title: 'Session ended',
        description: 'You signed out.',
      },
      loginSuccess: {
        title: 'Welcome!',
        description: 'Authentication completed successfully.',
      },
      loginError: {
        title: 'Authentication error',
      },
    },
    login: {
      brand: {
        badge: 'JF',
        title: 'Jornafy',
        subtitle: 'Time & attendance',
      },
      title: 'Sign in to your account',
      subtitle: 'Access the management panel securely.',
      emailLabel: 'Work email',
      emailPlaceholder: 'firstname.lastname@company.com',
      passwordLabel: 'Password',
      passwordPlaceholder: '********',
      show: 'Show',
      hide: 'Hide',
      togglePasswordAlt: 'Toggle password visibility',
      forgotPassword: 'Forgot password',
      submit: 'Sign in',
      submitting: 'Signing in...',
      sessionCopy: 'Secure session - Jornafy',
      footerBadge: 'JF',
      error: {
        title: 'Authentication failed',
        description: 'Invalid credentials. Please check and try again.',
      },
    },
    forgotPassword: {
      title: 'Forgot password',
      subtitle: 'Enter your account email to receive a verification code.',
      emailLabel: 'Email',
      submit: 'Send code',
      submitting: 'Sending...',
      successTitle: 'Code sent',
      successDescription: 'We sent a code to your email.',
      errorTitle: 'Unable to send code',
      errors: {
        emailRequired: 'Enter your account email.',
      },
    },
    activateAccount: {
      title: 'Activate account',
      subtitle: 'Create your password to access the system',
      email: 'Email',
      code: 'Activation code',
      password: 'New password',
      passwordConfirm: 'Confirm password',
      submit: 'Activate account',
      success: 'Account activated successfully',
      error: 'Could not activate the account',
    },
    resetPassword: {
      title: 'Reset password',
      subtitle: 'We sent a code to your email. Enter it to reset your password.',
      codeLabel: 'Verification code',
      codePlaceholder: 'XXXXXXXX',
      passwordLabel: 'New password',
      passwordConfirmLabel: 'Confirm password',
      passwordHelper: 'Use at least 8 characters with uppercase letters, numbers, and symbols.',
      strength: {
        label: 'Password strength',
        weak: 'Weak',
        medium: 'Medium',
        strong: 'Strong',
      },
      submit: 'Reset password',
      submitting: 'Resetting...',
      successTitle: 'Password updated',
      successDescription: 'Your password has been updated successfully.',
      errorTitle: 'Unable to reset password',
      errors: {
        codeRequired: 'Enter the verification code.',
        passwordRules:
          'Password must be at least 8 characters and include an uppercase letter, a number, and a symbol.',
        passwordContainsEmail: 'Password cannot contain your email.',
        passwordMismatch: 'Passwords do not match.',
      },
    },
    timeClock: {
      title: 'Time clock',
      subtitle: "Quickly check today's punches before you go.",
      status: {
        label: {
          heading: 'Day status',
          idle: 'Not started',
          working: 'In progress',
          break: 'On break',
          finished: 'Finished',
        },
        title: {
          idle: 'No entry yet',
          working: 'Shift in progress',
          break: 'Break in progress',
          finished: 'Day finished',
        },
        description: {
          idle: "You haven't clocked in today.",
          working: 'Shift active. Log breaks or clock-out when done.',
          break: 'Break running. Come back when you are ready.',
          finished: 'Day closed. Start a new entry tomorrow.',
        },
      },
      actions: {
        checkIn: 'Clock-in',
        checkOut: 'Clock-out',
        goToBreak: 'Start break',
        backFromBreak: 'Return from break',
        backToWork: 'Resume shift',
        goDashboard: 'Go to the dashboard',
        registerPoint: 'Register point',
        logout: 'Logout',
        registering: 'Registering...',
        locating: 'Getting location...',
      },
      summary: {
        title: 'Day summary',
        subtitle: 'Fast glance at planned vs. recorded time.',
        planned: 'Planned hours',
        recorded: 'Recorded hours',
        bank: 'Time bank (month)',
        todayBadge: 'Today',
      },
      lastRecord: {
        title: 'Last record',
        placeholder: 'No record found for today',
        label: '{{type}} at {{time}}',
        yesterdayLabel: '{{type}} at {{time}} (yesterday)',
      },
      lastPunch: {
        title: 'Last punch',
        registeredAt: 'Registered at: {{time}}',
        adjustmentTag: 'Adjustment',
        adjustmentPendingTooltip: 'Adjustment request pending approval',
        adjustRequest: 'Request adjustment',
        details: 'Details',
        opened: 'Opened',
        none: 'No punches yet',
        segment: 'Segment',
        sections: {
          first: 'First punch',
          interval: 'Interval',
          last: 'Last punch',
        },
      },
      recent: {
        title: 'Recent punches',
        viewAll: 'View all',
        entryLabel: 'Entry',
        exitLabel: 'Exit',
        breakStart: 'Break start',
        breakEnd: 'Break end',
        interval: '{{entryLabel}} {{start}} to {{exitLabel}} {{end}}',
        empty: 'No records found.',
      },
      syncedMessage: 'Entries sync automatically with the server.',
      help: 'Help',
      preferences: 'Preferences',
      breakGuard: {
        title: 'Clock in before starting a break',
        description: 'Start your shift to open a break.',
      },
      breakError: 'Could not register break',
      breakErrorDescription: 'Try again in a moment.',
      openEntryWarning: {
        title: 'Open time entry',
        description: 'You have an open punch that must be closed to regularize your day.',
        cta: 'Resolve now',
        nextAction: 'Next action: {{action}}',
      },
      nextActions: {
        clock_in: 'Clock in',
        clock_out: 'Clock out',
        break_start: 'Start break',
        break_end: 'End break',
        default: 'Register punch',
      },
      absence: {
        typeFallback: 'Absence',
        commentFallback: 'No justification provided.',
        blockTitle: 'Clocking blocked',
        blockDescription: 'You are marked absent today and cannot register a punch.',
        bannerTitle: 'You are absent today:',
        periodLabel: 'Period:',
        reasonLabel: 'Reason:',
        blockedLabel: 'You are blocked from registering a punch.',
      },
      geolocation: {
        requiredTitle: 'Location required',
        errors: {
          permissionDenied:
            'Location is required to register this punch. Allow access to your location and try again.',
          permissionBlocked:
            'Location permission has already been blocked in the browser. Enable it in the site settings to register this punch.',
          timeout:
            'We could not get your location in time. Location is required to register this punch.',
          unavailable:
            'We could not get your location. Location is required to register this punch.',
          unsupported:
            'Your device or browser does not support geolocation. Location is required to register this punch.',
        },
      },
    },
    dashboardPage: {
      title: 'Dashboard',
      badges: {
        today: 'Today',
      },
      labels: {
        hoursLabel: 'Time: {{time}}',
      },
      header: {
        toggleMenu: 'Toggle sidebar',
        closeMenu: 'Close menu',
      },
      searchPlaceholder: 'Search teammate, team, or project',
      nav: {
        dashboard: 'Dashboard',
        history: 'Punch history',
        documents: 'Documents',
        employees: 'Employees',
        calendar: 'Calendar',
        registerPoint: 'Register point',
        timeOff: 'Time Off',
        projects: 'Projects',
        team: 'Team',
        equipo: 'Employees',
        settings: 'Settings',
      },
      version: {
        label: 'Version 1.0',
        product: 'Jornafy',
      },
      todayPill: '{{time}} Today - {{date}}',
      timeTracking: {
        tag: 'Time tracking',
        title: 'Punch history',
        subtitle: '{{total}} logged this month - {{extra}} extra',
        empty: 'No records found.',
        entryLabel: 'Clock-in',
        exitLabel: 'Clock-out',
        interval: '{{entryLabel}} {{start}} - {{exitLabel}} {{end}}',
        status: {
          normal: 'On track',
          extra: 'Overtime',
          late: 'Late',
        },
        actions: {
          fullHistory: 'Full history',
          adjust: 'Adjust punch',
        },
      },
      documents: {
        tag: 'Documents',
        title: 'Team member documents',
        subtitle: 'Quick preview of key folders.',
        emptyState: 'No documents available.',
        emptyStateSecondary: 'No documents have been uploaded yet.',
        defaultAction: 'Open',
        total_one: '{{count}} file',
        total_other: '{{count}} files',
        sections: {
          payroll: {
            title: 'Payroll',
            description: 'Latest payslips.',
            items: {
              current: {
                name: 'Payslip - November 2025',
                status: 'Available',
                updatedAt: 'Generated on 11/30',
                action: 'Download',
              },
              previous: {
                name: 'Payslip - October 2025',
                status: 'Signed',
                updatedAt: 'Signed on 10/31',
                action: 'View receipt',
              },
            },
          },
          courses: {
            title: 'Courses',
            description: 'Required trainings and development tracks.',
            items: {
              onboarding: {
                name: 'Security onboarding',
                status: 'Due on 12/22',
                updatedAt: 'Expiring soon',
                action: 'Review',
              },
              lgpd: {
                name: 'Data protection training',
                status: 'Completed',
                updatedAt: '10/12',
                action: 'Review',
              },
            },
          },
          personal: {
            title: 'Personal documents',
            description: 'Employee records and proofs.',
            items: {
              address: {
                name: 'Proof of address',
                status: 'Uploaded just now',
                updatedAt: 'Last upload',
                action: 'View',
              },
              rg: {
                name: 'ID - updated',
                status: 'Verified',
                updatedAt: '08/04',
                action: 'View',
              },
              workCard: {
                name: 'Digital work card',
                status: 'Linked',
                updatedAt: 'Synced',
                action: 'Open',
              },
            },
          },
        },
      },
      announcements: {
        tag: 'Company',
        title: 'Announcements',
        status: {
          seen: 'Seen',
          pending: 'Pending',
        },
        items: {
          hybrid: {
            title: 'Hybrid model update',
            body: 'Starting September, 3 home office days per week for admin teams.',
            sentAt: 'Sent on Aug 12 - 17:42 - HQ',
            viewedAt: 'Seen on Aug 12 at 18:10',
          },
          security: {
            title: 'Security policy refreshed',
            body: 'Review the new password and MFA guide by month-end.',
            sentAt: 'Sent on Aug 10 - 09:15 - Jornafy',
            viewedAt: 'Seen on Aug 10 at 10:02',
          },
        },
      },
      timeOff: {
        tag: 'Time Off',
        title: 'Time off and absences',
        subtitle: 'Updated balance for the current period.',
        gaugeLabel: 'Days',
        available: 'Available balance',
        availableSuffix: 'days',
        nextVacation: 'Next vacation',
        nextVacationValue: 'Aug 21-28',
        status: 'Status',
        statusValue: {
          approved: 'Approved',
          pending: 'Pending',
          not_requested: 'Not requested',
        },
        absences: 'Recent absences',
        absencesValue_one: '{{count}} record',
        absencesValue_other: '{{count}} records',
        request: 'Request vacation',
      },
      toasts: {
        fullHistory: {
          title: 'Full history',
          description: 'Opening detailed view of all records.',
        },
        documentAction: {
          title: 'Team document',
          description: '{{action}}: {{name}}',
          defaultAction: 'Opening',
        },
        documents: {
          title: 'Documents',
          description: 'Showing all files and proofs for the team member.',
        },
        profile: {
          title: 'Team member profile',
          description: 'Opening colleague profile.',
        },
        help: {
          title: 'Get help',
          description: 'Redirecting to support.',
        },
        timeOff: {
          title: 'Time off and absences',
          description: 'Showing full vacations and absences page.',
        },
        vacation: {
          title: 'Vacation request',
          description: 'Opening full vacation request screen.',
        },
        announcements: {
          title: 'Announcements',
          description: 'Showing recent announcements.',
        },
      },
      common: {
        viewAll: 'View all',
      },
    },
    announcementsPage: {
      tag: 'Company',
      title: 'Announcements',
      subtitle: 'Updates shared by Admin and Area Manager.',
      helper: {
        updated: 'Synced with Jornafy',
        pendingBadge: '{{count}} pending',
      },
      actions: {
        refresh: 'Refresh',
        retry: 'Try again',
        confirm: 'Confirm view',
        confirming: 'Confirming...',
      },
      filters: {
        title: 'Filters',
        subtitle: 'Filter announcements by status.',
        tabs: {
          all: 'All',
          pending: 'Pending',
          seen: 'Seen',
        },
      },
      sort: {
        label: 'Sort',
        recent: 'Most recent',
      },
      labels: {
        sender: 'Sender: {{sender}}',
        sentAt: 'Sent on {{date}} at {{time}}',
        sentAtDateOnly: 'Sent on {{date}}',
        sentAtFallback: 'Send date unavailable',
      },
      status: {
        pending: 'Pending',
        seen: 'Seen',
        seenAt: 'Seen on {{date}} at {{time}}',
        seenAtDateOnly: 'Seen on {{date}}',
      },
      states: {
        empty: 'No announcements found.',
        emptyPending: 'No pending announcements.',
        error: 'Unable to load announcements.',
        detailError: 'Unable to load announcement details.',
      },
      modal: {
        title: 'Announcement details',
        description: 'Review the full message shared by the team.',
      },
      toasts: {
        markError: {
          title: 'Confirmation failed',
          description: 'Unable to register the view. Please try again.',
        },
      },
      sender: {
        admin: 'Admin',
        areaManager: 'Area Manager',
        default: 'Team',
      },
    },
    documentsPage: {
      tag: 'Documents',
      title: 'Employee documentation',
      subtitle: 'Keep payslips, trainings, and proofs in one place.',
      helper: {
        between: 'Updated in the last 30 days',
        badge: 'Synced with Jornafy',
      },
      actions: {
        exportCsv: 'Export CSV',
        exportPdf: 'Export PDF',
        upload: 'Upload document',
        view: 'View',
        download: 'Download',
        renew: 'Renew',
      },
      filters: {
        title: 'Filters',
        description: 'Filter by type, status, and period.',
        tag: 'Dynamic',
        category: 'Document type',
        status: 'Status',
        segmented: {
          all: 'All',
          available: 'Available',
          pending: 'Pending',
          review: 'In review',
          expired: 'Expired',
        },
        range: 'Period',
        rangeOptions: {
          last30: 'Last 30 days',
          thisMonth: 'This month',
          thisYear: 'This year',
          custom: 'Custom',
        },
        reset: 'Reset',
        apply: 'Apply filters',
      },
      toasts: {
        exportCsv: {
          title: 'CSV ready',
          description: 'All visible documents are included.',
        },
        exportPdf: {
          title: 'PDF ready',
          description: 'Download the filtered view as PDF.',
        },
        upload: {
          title: 'Upload ready',
          description: 'File queued. Send when ready.',
        },
        uploadReady: {
          title: 'Upload ready',
          description: 'File queued. Send when ready.',
        },
        filter: {
          title: 'Filters applied',
          description: 'The list is now up to date.',
        },
      },
      upload: {
        title: 'Quick upload',
        description: 'Drag & drop or select a file and assign a category.',
        category: 'Category',
        fileTypes: 'PDF • JPG',
        dropActiveTitle: 'Drop files to upload',
        dropActiveDescription: 'Files received. Hit send to finish.',
        dropTitle: 'Drag your file here',
        dropDescription: 'Max 10MB • PDF, JPG, PNG.',
        browse: 'Choose file',
        send: 'Send',
        tip: 'Use consistent names like "Payslip - Nov 2025".',
      },
      table: {
        label: 'Table',
        title: 'Full view',
        search: 'Search by name, type, or status',
        empty: 'No documents match the filters.',
        headers: {
          document: 'Document',
          type: 'Type',
          status: 'Status',
          date: 'Date',
          action: 'Action',
        },
      },
      status: {
        available: 'Available',
        pending: 'Pending',
        review: 'In review',
        expired: 'Expired',
        pending_manager: 'Awaiting signature',
      },
      tabs: {
        all: 'All',
        payroll: 'Payroll',
        courses: 'Courses',
        personal: 'Personal',
        others: 'Others',
      },
      priority: {
        high: 'High',
        medium: 'Medium',
        low: 'Low',
        none: 'No priority',
      },
      signature: {
        pending: 'Signature pending',
        signed: 'Signed',
        notRequired: 'No signature required',
      },
      views: {
        seenAt: 'Seen on {{date}}',
        notSeen: 'Not seen yet',
      },
      employee: {
        title: 'My documents',
        subtitle: 'View and download your documents whenever you need.',
        listCount: '{{count}} documents',
        emptyState: 'No documents available yet.',
        filters: {
          searchPlaceholder: 'Search documents',
        },
        pagination: {
          previous: 'Previous',
          next: 'Next',
        },
        table: {
          headers: {
            document: 'Document',
            category: 'Category',
            status: 'Status',
            type: 'Type',
            size: 'Size',
            actions: 'Actions',
          },
        },
        actions: {
          refresh: 'Refresh',
          resend: 'Resend',
        },
        fileLabel: 'File',
        fileFallback: 'Document',
        updatedAt: 'Updated on {{date}}',
        resend: {
          title: 'Resend document',
          description: 'Upload a new file to replace the document currently in review.',
          fileLabel: 'New file',
          fileHint: 'Use the corrected file to move the document back to pending.',
          sending: 'Resending...',
          submit: 'Resend document',
        },
        errors: {
          sessionExpired: 'Your session expired. Please sign in again.',
          noPermission: "You don't have permission to view this document.",
          notFound: 'Document not found.',
          viewGeneric: "We couldn't open the document.",
          downloadGeneric: "We couldn't download the document.",
        },
        toasts: {
          loadErrorTitle: 'Load error',
          loadErrorDescription: "We couldn't load the documents.",
          downloadErrorTitle: 'Download error',
          downloadErrorDescription: "We couldn't download the document.",
          viewErrorTitle: 'Open error',
          resendSuccessTitle: 'Document resent',
          resendSuccessDescription: 'The document was resent for a new review.',
          resendErrorTitle: 'Resend error',
          resendErrorDescription: "We couldn't resend the document.",
          resendFileRequiredTitle: 'File required',
          resendFileRequiredDescription: 'Select a file before resending the document.',
        },
      },
      admin: {
        title: 'Document management',
        subtitle: 'Upload documents and track the team status.',
        listCount: '{{count}} documents',
        tabs: {
          all: 'All',
          pending: 'Pending',
          review: 'In review',
        },
        filters: {
          searchPlaceholder: 'Search documents',
          employeePlaceholder: 'Search employee',
          selectedCount: '{{count}} employee(s) selected',
        },
        pagination: {
          previous: 'Previous',
          next: 'Next',
        },
        table: {
          headers: {
            employee: 'Employee',
            title: 'Document',
            category: 'Category',
            priority: 'Priority',
            signature: 'Signature',
            lastView: 'Last view',
            status: 'Status',
            actions: 'Actions',
          },
        },
        labels: {
          employeeFallback: 'No employee',
          updatedAt: 'Updated on {{date}}',
          priority: 'Priority: {{value}}',
          absenceType: 'Absence: {{type}}',
          timesheetPeriod: 'Timesheet - {{month}}/{{year}}',
        },
        fileFallback: 'Document',
        actions: {
          refresh: 'Refresh',
          upload: 'Upload document',
          approve: 'Approve',
          reject: 'Reject',
          rejecting: 'Rejecting...',
          sign: 'Sign',
          signing: 'Signing...',
        },
        upload: {
          title: 'Upload document',
          description: 'Attach a file and fill in the employee document details.',
          employeeLabel: 'Employee',
          employeePlaceholder: 'Select an employee',
          employeeLoading: 'Loading employees...',
          employeeEmpty: 'No employees found.',
          categoryLabel: 'Category',
          categoryPlaceholder: 'Select a category',
          titleLabel: 'Title',
          titlePlaceholder: 'Ex.: May payslip',
          notesLabel: 'Notes',
          notesPlaceholder: 'Add optional context for the employee.',
          fileLabel: 'File',
          fileHint: 'Accepted formats: PDF, JPG or PNG.',
          sending: 'Uploading...',
          submit: 'Upload document',
          successTitle: 'Document uploaded',
          successDescription: 'The document was uploaded successfully.',
          errorTitle: 'Upload error',
          errorDescription: "We couldn't upload the document.",
          errors: {
            fileTitle: 'File required',
            fileDescription: 'Select a file to continue.',
            employeeTitle: 'Employee required',
            employeeDescription: 'Select an employee to continue.',
            categoryTitle: 'Category required',
            categoryDescription: 'Select a category.',
          },
        },
        toasts: {
          loadErrorTitle: 'Load error',
          loadErrorDescription: "We couldn't load the documents.",
          viewErrorTitle: 'Open error',
          downloadErrorTitle: 'Download error',
          approveSuccessTitle: 'Document approved',
          approveSuccessDescription: '{{title}} was approved.',
          approveErrorTitle: 'Approve error',
          approveErrorDescription: "We couldn't approve the document.",
          rejectSuccessTitle: 'Document rejected',
          rejectSuccessDescription: '{{title}} was rejected.',
          rejectErrorTitle: 'Reject error',
          rejectErrorDescription: "We couldn't reject the document.",
          signSuccessTitle: 'Timesheet signed',
          signSuccessDescription: 'The timesheet was signed successfully.',
          signErrorTitle: 'Sign error',
          signErrorDescription: "We couldn't sign the timesheet.",
        },
        errors: {
          viewGeneric: "We couldn't open the document.",
          downloadGeneric: "We couldn't download the document.",
        },
        reject: {
          title: 'Reject document',
          description: 'Provide a reason to notify the employee.',
          placeholder: 'Describe the rejection reason...',
          requiredTitle: 'Reason required',
          requiredDescription: 'Please provide a reason (min 5 characters).',
        },
        states: {
          empty: 'No documents in this section.',
          noPermission: "You don't have permission to view these documents.",
        },
      },
    },
    employeesPage: {
      tag: 'Employees',
      title: 'Employee management',
      subtitle: 'Invite teammates, manage roles, and assign shifts.',
      helper: {
        primary: 'Access and roles',
        secondary: 'Admin workspace',
      },
      actions: {
        new: 'New employee',
        refresh: 'Refresh',
        edit: 'Edit',
        shift: 'Shift',
        delete: 'Delete',
        create: 'Create',
        save: 'Save',
        assignShift: 'Assign shift',
      },
      summary: {
        total: 'Total employees',
        managers: 'Admins and managers',
        employees: 'Employees',
      },
      filters: {
        title: 'Filters',
        subtitle: 'Search by name/email and role.',
        search: 'Search',
        searchPlaceholder: 'Search by name or email',
        role: 'Role',
      },
      table: {
        label: 'Table',
        title: 'Employee list',
        empty: 'No employees found.',
        columns: {
          name: 'Name',
          email: 'Email',
          role: 'Role',
          shift: 'Shift',
          createdAt: 'Created at',
          actions: 'Actions',
        },
        company: 'Company',
        noShift: '-',
      },
      roles: {
        all: 'All roles',
        admin: 'Admin',
        manager: 'Manager',
        area_manager: 'Area manager',
        employee: 'Employee',
      },
      form: {
        name: 'Name',
        email: 'Email',
        passwordOptional: 'Password (optional)',
        shift: 'Shift (optional)',
        role: 'Role',
        startDate: 'Start date',
      },
      modals: {
        create: { title: 'New employee' },
        edit: { title: 'Edit employee' },
        shift: { title: 'Assign shift' },
        delete: {
          title: 'Remove employee',
          description: 'Do you want to remove {{name}}?',
        },
      },
      toasts: {
        created: {
          title: 'Invitation sent',
          description: 'Employee created successfully.',
        },
        updated: {
          title: 'Employee updated',
          description: 'Changes saved.',
        },
        deleted: {
          title: 'Employee removed',
          description: 'Account deleted successfully.',
        },
        shiftAssigned: {
          title: 'Shift assigned',
          description: 'Shift updated for the employee.',
        },
      },
      errors: {
        generic: 'Unable to complete the action.',
        emailTaken: 'Email already in use.',
      },
      states: {
        loading: 'Loading employees...',
      },
      pagination: {
        previous: 'Previous',
        next: 'Next',
        page: 'Page',
      },
      shifts: {
        morning: 'Morning (08:00 - 17:00)',
        evening: 'Afternoon (13:00 - 22:00)',
        overnight: 'Overnight (22:00 - 06:00)',
      },
    },
    equipoPage: {
      tag: 'Admin',
      title: 'Team',
      subtitle: 'Employee management',
      searchPlaceholder: 'Search by name or email',
      filters: {
        areaLabel: 'Filter by area',
        allAreas: 'All areas',
      },
      sort: {
        label: 'Sort list',
        columnAriaLabel: 'Sort by {{column}}',
        options: {
          'createdAt:desc': 'Newest date',
          'createdAt:asc': 'Oldest date',
          'area:asc': 'Area (A-Z)',
          'role:asc': 'Role',
        },
      },
      table: {
        label: 'Employees',
        title: 'Employee management',
        count: '{{count}} people',
        emptyTitle: 'No employees found',
        emptyDescription: 'Try a different search or add a new employee.',
        emptyName: 'Name unavailable',
        emptyEmail: 'Email unavailable',
        emptyArea: 'No area',
        emptyDate: 'No date',
        headers: {
          row: '#',
          name: 'Name',
          email: 'Email',
          role: 'Role',
          area: 'Area',
          createdAt: 'Created at',
          deletedAt: 'Deactivated at',
          actions: 'Actions',
        },
      },
      tabs: {
        active: 'Active',
        inactive: 'Inactive',
      },
      badges: {
        shiftAssigned: 'Shift assigned',
        noRole: 'No role',
      },
      pagination: {
        page: 'Page {{page}}',
        pageOf: 'Page {{page}} of {{total}}',
        previous: 'Previous',
        next: 'Next',
      },
      actions: {
        create: 'New employee',
        refresh: 'Refresh',
        save: 'Save',
        creating: 'Creating...',
        updating: 'Saving...',
        openMenu: 'Open actions',
        edit: 'Edit',
        assignShift: 'Assign shift',
        resendFirstAccessEmail: 'Resend first access email',
        resendingFirstAccessEmail: 'Resending first access email...',
        assign: 'Assign',
        assigning: 'Assigning...',
        deactivate: 'Deactivate',
        deactivating: 'Removing...',
        reactivate: 'Reactivate',
        reactivating: 'Reactivating...',
        retry: 'Try again',
      },
        form: {
          nameLabel: 'Name',
          emailLabel: 'Email',
          roleLabel: 'Role',
          areaLabel: 'Primary area',
          areaPlaceholder: 'Select the primary area (optional)',
          managedAreasLabel: 'Managed areas',
          managedAreasPlaceholder: 'Select one or more areas',
          managedAreasSearchPlaceholder: 'Search area',
          managedAreasEmpty: 'No areas found.',
          managedAreasClear: 'Clear',
          managedAreasSelectedCount: '{{count}} areas selected',
          managedAreasHint: 'Open the list and click the areas you want to manage.',
          shiftLabel: 'Shift',
        shiftPlaceholder: 'Select a shift',
        passwordLabel: 'Password (optional)',
        passwordPlaceholder: 'Set a new password',
        startDateLabel: 'Start date',
      },
      modals: {
        createTitle: 'New employee',
        createDescription: 'Create a new employee and send an invite.',
        editTitle: 'Edit employee',
        editDescription: 'Update employee details.',
        assignTitle: 'Assign shift',
        assignDescription: 'Choose a shift and start date.',
        deleteTitle: 'Deactivate employee',
        deleteDescription: 'Remove {{name}} from the team?',
        restoreTitle: 'Reactivate employee',
        restoreDescription: 'Are you sure you want to reactivate {{name}}? Their system access will be restored.',
      },
      roles: {
        all: 'All roles',
        admin: 'Admin',
        manager: 'Manager',
        area_manager: 'Area manager',
        employee: 'Employee',
        unknown: 'Unknown',
      },
      shifts: {
        loading: 'Loading shifts...',
        empty: 'No shifts available',
        unnamed: 'Unnamed shift',
      },
      areas: {
        loading: 'Loading areas...',
        empty: 'No areas available',
        unnamed: 'Unnamed area',
      },
      extraEmployees: {
        pendingTitle: 'Pending payment for extra counted users',
        overdueTitle: 'Overdue payment for extra counted users',
        badges: {
          pending: 'Pending',
          overdue: 'Overdue',
        },
        description:
          'There are {{count}} extra counted user(s) waiting for payment. Once paid, this quantity becomes purchased extra capacity for the company.',
        pendingQuantity: '{{count}} pending',
        paidAllowance: '{{count}} paid extras',
        dueAt: 'Due on {{date}}',
        payAction: 'Pay in Stripe',
        redirecting: 'Redirecting...',
        errors: {
          title: 'Could not open checkout',
          description: 'Please try again in a moment.',
          missingUrl: 'Checkout response did not include a valid URL.',
        },
      },
      toasts: {
        loadError: {
          title: 'Failed to load employees',
        },
        shiftsError: {
          title: 'Failed to load shifts',
          description: 'We could not load shifts.',
        },
        areasError: {
          title: 'Failed to load areas',
          description: 'We could not load areas.',
        },
        createSuccess: {
          title: 'Employee created',
          description: 'Invite sent successfully.',
        },
        resendInviteSuccess: {
          title: 'Email resent',
          description: 'The first access email was resent successfully.',
        },
        createError: {
          title: 'Failed to create employee',
          description: 'We could not create the employee.',
        },
        resendInviteError: {
          title: 'Failed to resend email',
          description: 'We could not resend the first access email.',
        },
        detailsError: {
          title: 'Failed to load employee',
          description: 'We could not load the employee details.',
        },
        updateSuccess: {
          title: 'Employee updated',
          description: 'Details updated successfully.',
        },
        updateError: {
          title: 'Failed to update employee',
          description: 'We could not update the employee.',
        },
        deleteSuccess: {
          title: 'Employee removed',
          description: 'Employee removed successfully.',
        },
        deleteError: {
          title: 'Failed to remove employee',
          description: 'We could not remove the employee.',
        },
        assignSuccess: {
          title: 'Shift assigned',
          description: 'Shift assigned successfully.',
        },
        assignError: {
          title: 'Failed to assign shift',
          description: 'We could not assign the shift.',
        },
        restoreSuccess: {
          title: 'Employee reactivated',
          description: 'The employee has been reactivated successfully.',
        },
        restoreSuccessNoRole: {
          title: 'Employee reactivated',
          description: 'Reactivated without a role. Please set the role in the edit screen.',
        },
        restoreError: {
          title: 'Failed to reactivate',
          description: 'We could not reactivate the employee.',
        },
      },
      states: {
        errorTitle: 'Failed to load employees',
        errorDescription: 'We could not load employees.',
        noPermissionTitle: 'Access denied',
        noPermissionDescription: 'You do not have access to this page.',
      },
    },    historyPage: {
      badge: 'Punch history',
      subBadge: 'Employee: {{name}}',
      title: 'Time entry history',
      subtitle: 'See every punch, filter by date, export, and request adjustments.',
      export: {
        label: 'Export CSV',
        emptyTitle: 'Nothing to export',
        emptyDescription: 'Apply another filter or load more records before exporting.',
        successTitle: 'Export ready',
        successDescription: '{{count}} record(s) added to the file.',
        pdfLabel: 'Export PDF',
      },
      filters: {
        title: 'Date filters',
        description: 'Pick a range and reload the list.',
        month: 'Month',
        from: 'From',
        to: 'To',
        quick: {
          lastMonth: 'Last month',
          thisMonth: 'This month',
          last7Days: 'Last 7 days',
        },
        apply: 'Apply filters',
        clear: 'Clear',
      },
      helper: {
        title: 'Quick tips',
        description: 'The list honors the selected range and you can request adjustments per row.',
      },
      states: {
        errorTitle: 'Could not load history',
        errorDescription: 'Something went wrong loading the records.',
        retry: 'Try again',
        emptyTitle: 'No records found',
        emptyDescription: 'Adjust filters or record a new punch.',
      },
      actions: {
        loadMore: 'Load more',
        loadingMore: 'Loading...',
        back: 'Back',
      },
      labels: {
        duration: 'Day duration: {{value}}',
        noDuration: 'No calculated duration',
        totalEntries_one: '{{count}} record',
        totalEntries_other: '{{count}} records',
        timeFallback: '--:--',
        emptyValue: '—',
      },
      table: {
        headers: {
          breakReturn: 'Back from break',
        },
      },
      adjustment: {
        cta: 'Request adjustment',
        modalTitle: 'Request adjustment',
        modalDescription: 'Review the record and enter the correct time.',
        date: 'Date',
        originalTime: 'Original time',
        type: 'Type',
        desiredTime: 'Desired time',
        reason: 'Reason',
        reasonPlaceholder: 'Explain what happened...',
        submit: 'Send request',
        submitting: 'Sending...',
        errorTitle: 'Could not send adjustment',
        errorDescription: 'Please try again in a moment.',
        missingEntry: 'Select a record to adjust.',
        types: {
          breakStart: 'Break start',
          breakEnd: 'Break end',
        },
      },
    },

    closeTimesheetPage: {
      badge: 'Managers',
      defaultRange: 'Default: last month',
      title: 'Close timesheet',
      subtitle: 'Review and export punches for the selected period.',
      labels: {
        period: 'Period',
      },
      filters: {
        title: 'Filters',
        quick: {
          lastMonth: 'Last month',
          thisMonth: 'This month',
          last7Days: 'Last 7 days',
        },
        employee: 'Employee',
        searchPlaceholder: 'Search by name or email',
        selectedCount: '{{count}} employee(s) selected',
        empty: 'Select an employee to view.',
        from: 'Start date',
        to: 'End date',
        hint: 'Pick an employee and a complete date range.',
        multiRangeHint:
          'When multiple employees are selected, the period is limited to the last 7 days.',
        reset: 'Clear',
        submit: 'Search',
      },
      states: {
        employeesError: 'Could not load the employees list.',
        employeesErrorTitle: 'Failed to load employees',
        entriesError: 'Could not load entries for this period.',
        entriesErrorTitle: 'Failed to load entries',
        empty: 'No records for this period.',
        waitingEmployee: 'Select an employee and valid dates to continue.',
      },
      summary: {
        title: 'Summary',
        value: 'Value',
        entries: 'Total punches',
        overtime: 'Overtime',
        periodOvertime: 'Overtime in period',
        days: 'Days with records',
        hours: 'Total hours',
        hoursOnly: '{{count}} hours',
        inconsistencies: 'Issues',
        pending: 'Pending',
        duplicates: 'Duplicated',
        pendingCount: '{{count}} clock-in(s) without clock-out',
        duplicateCount: '{{count}} duplicated record(s)',
      },
      export: {
        primary: 'Export PDF',
        detailed: 'Export detailed PDF',
        emptyTitle: 'Nothing to export',
        emptyDescription: 'Run a search before exporting.',
        pageCounter: 'Page {{current}} of {{total}}',
        title: 'Timesheet',
        company: 'Company',
        shift: 'Shift',
        employee: 'Employee',
        period: 'Period',
        issuedAt: 'Issued at',
        columns: {
          date: 'Date',
          start: 'Clock-in',
          breakStart: 'Break start',
          breakEnd: 'Break end',
          end: 'Clock-out',
        },
        shiftFallback: 'No shift linked',
        emptySlot: '—',
        summary: {
          daysWithRecords: '{{count}} day(s) with records',
          totalEntries: '{{count}} punch(es)',
        },
        multipleEmployeesTitle: 'Export unavailable',
        multipleEmployeesDescription:
          'Select only one employee to export the timesheet as PDF.',
        successTitle: 'PDF generated',
        successDescription: 'Download started successfully.',
        errorTitle: 'Export error',
        errorDescription: 'Could not generate the PDF.',
      },
      table: {
        title: 'Detailed punches',
        subtitle: 'Compact date-based view for manager review.',
        noTime: 'No time',
        unknownDate: 'Unknown date',
        noDate: 'No date',
        noType: 'No type',
        userFallback: 'Employee',
        viewLocation: 'View location',
        locationTitle: 'Clock-in location',
        locationDescription: 'Coordinates captured for this punch.',
        distanceFromCompany: 'Distance from company',
        distanceValue: '{{distance}} m',
        openMaps: 'Open in Maps',
        records: 'records',
        pagination: 'Page {{page}} of {{total}}',
        prev: 'Previous',
        next: 'Next',
        headers: {
          date: 'Date',
          time: 'Time',
          type: 'Type',
          user: 'User',
          location: 'Location',
          notes: 'Status',
          actions: 'Actions',
          status: 'Status',
          source: 'Source',
        },
        status: {
          adjustmentPending: 'Adjustment pending',
          pending: 'Open punch',
          duplicate: 'Duplicated',
          ok: 'Complete',
        },
        absence: {
          badge: 'Absence',
          timeLabel: 'Absence in period',
          groupMeta: 'Absence recorded for the day',
          locationFallback: 'No location punch recorded',
          typeFallback: 'Recorded absence',
          commentFallback: 'No additional note for this absence.',
          coverage: {
            hours: 'Hourly coverage',
            days: 'Daily coverage',
          },
          types: {
            excused: 'Excused absence',
            unjustified: 'Unjustified absence',
            medical: 'Medical leave',
            medicalCertificate: 'Medical certificate',
            vacation: 'Vacation',
          },
        },
        locationStatus: {
          inside: 'Inside company',
          outside: 'Outside company',
        },
        dailyTotal: 'worked',
      },
      delete: {
        action: 'Delete',
        confirm: 'Delete',
        deleting: 'Deleting...',
        title: 'Delete time entry',
        description:
          'Are you sure you want to delete this time entry? This action will remove the punch from listings and reports.',
        successTitle: 'Entry deleted',
        successDescription: 'Time entry deleted successfully.',
        errorTitle: 'Error deleting entry',
        forbiddenDescription: 'You do not have permission to delete this entry.',
        notFoundDescription: 'Entry not found or already removed.',
        errorDescription: 'Could not delete the time entry.',
      },
    },
    adminMonthlyClosuresPage: {
      badge: 'Administration',
      title: 'Monthly closure',
      subtitle: 'Manage monthly timesheet closures for your team.',
      actions: {
        refresh: 'Refresh',
        closePeriod: 'Close month',
        collapse: 'Collapse',
        expand: 'View timesheets',
        sign: 'Sign',
        signing: 'Signing...',
        resolveDispute: 'Resolve dispute',
        closeSuccessTitle: 'Month closed',
        closeSuccessDescription: 'The closure has been initiated. Snapshots will be generated shortly.',
        closeError: 'Could not close the month.',
        signSuccessTitle: 'Timesheet signed',
        signSuccessDescription: 'The signature has been recorded successfully.',
        signErrorTitle: 'Error signing',
      },
      form: {
        title: 'Close period',
        description: 'Select the month and year to start the timesheet monthly closure.',
        monthYearLabel: 'Reference month',
        selectMonth: 'Select a month',
        selectError: 'Please select a month and year.',
        submit: 'Close period',
        submitting: 'Processing...',
      },
      status: {
        processing: 'Processing',
        open: 'Open',
        completed: 'Completed',
      },
      timesheetStatus: {
        pending_employee: 'Awaiting employee',
        disputed: 'Disputed',
        pending_manager: 'Awaiting manager',
        completed: 'Completed',
      },
      dispute: {
        openLabel: 'Open dispute',
        reason: 'Reason',
        resolveTitle: 'Resolve dispute',
        resolveDescription: 'Enter the resolution note. The timesheet will return to the employee for signature.',
        noteLabel: 'Resolution note',
        notePlaceholder: 'Describe how it was resolved...',
        noteRequired: 'Enter the resolution note.',
        submit: 'Resolve',
        submitting: 'Resolving...',
        resolveSuccessTitle: 'Dispute resolved',
        resolveSuccessDescription: 'The timesheet will return to the employee for signature.',
        resolveError: 'Could not resolve the dispute.',
      },
      table: {
        sigCount: '{{count}} signature(s)',
      },
      closure: {
        closedBy: 'Closed by {{name}}',
        timesheets: '{{count}} timesheet(s)',
      },
      states: {
        errorTitle: 'Failed to load',
        error: 'Could not load the closures.',
        errorTimesheets: 'Could not load the timesheets.',
        emptyTitle: 'No closures found',
        emptyDescription: 'Use the "Close month" button to start the first closure.',
        emptyTimesheets: 'No timesheets in this closure.',
      },
    },

    auth: {
      errors: {
        tokenMissing: 'Token not returned by API',
        loginFailed: 'Unable to log in. Check your credentials.',
        unauthorized: 'Invalid credentials. Please check and try again.',
      },
    },
    holidaysPage: {
      tag: 'Holidays',
      title: 'Holiday Management',
      subtitle: 'Create and manage company holidays.',
      actions: {
        refresh: 'Refresh',
        create: 'New holiday',
        edit: 'Edit',
        delete: 'Delete',
      },
      filters: {
        searchPlaceholder: 'Search by name...',
        startDate: 'Start date',
        endDate: 'End date',
      },
      scopes: {
        all: 'All scopes',
        national: 'National',
        regional: 'Regional',
        local: 'Local',
        company: 'Company',
      },
      table: {
        date: 'Date',
        name: 'Name',
        scope: 'Scope',
        actions: 'Actions',
      },
      listCount: '{{count}} holiday(s)',
      form: {
        createTitle: 'New holiday',
        editTitle: 'Edit holiday',
        createDescription: 'Fill in the details to create a new holiday.',
        editDescription: 'Update the holiday details.',
        dateLabel: 'Date',
        nameLabel: 'Name',
        namePlaceholder: 'e.g. Labor Day',
        scopeLabel: 'Scope',
        submit: 'Save',
        saving: 'Saving...',
      },
      delete: {
        title: 'Delete holiday',
        description: 'Are you sure you want to delete "{{name}}"? This action cannot be undone.',
        confirm: 'Delete',
        deleting: 'Deleting...',
      },
      states: {
        empty: 'No holidays found.',
        noPermission: 'You do not have permission to access this page.',
        loadError: 'Failed to load holidays.',
      },
      toasts: {
        loadErrorTitle: 'Load error',
        createSuccess: 'Holiday created successfully.',
        updateSuccess: 'Holiday updated successfully.',
        deleteSuccess: 'Holiday deleted successfully.',
        saveErrorTitle: 'Save error',
        saveError: 'Could not save the holiday.',
        deleteErrorTitle: 'Delete error',
        deleteError: 'Could not delete the holiday.',
      },
    },
  },
}

export default en

